...
class Variables {
  ...
  static const _sendTimeout = EnvEntry('SEND_TIMEOUT', '$_connectTimeoutValue');

  static const _baseUrl = EnvEntry('BASE_URL', 'https://health-dev.digit.org/');
  ...
  AppConfig toAppConfig() {
    return AppConfig(
      appConfig: [
        AppConfigListItems(
          tenantId: tenantId,
          languages: const [], // Add actual language data if available
          backendInterface: InterfacesList(
            // Add interface configuration if available
            interfaces: [Interfaces(name: 'MDMS', type: completeMdmsApiUrl)],
          ),
          genderOptions: const [], // Add gender options if available
        ),
      ],
    );
  }

  const Variables({this.useFallbackValues = false, required DotEnv dotEnv})
    : _dotEnv = dotEnv;
  ...
  int get sendTimeout =>
      useFallbackValues
          ...
          : int.tryParse(
                _dotEnv.get(_sendTimeout.key, fallback: _sendTimeout.value),
              ) ??
              _sendTimeoutValue;

  int get syncDownRetryCount =>
      useFallbackValues
